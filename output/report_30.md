...
*Generated: 2026-07-24T13:33:15.434Z*
*Mode: SUBSET (30 docs)*

## Executive Summary

- **Total Documents Processed**: 30
- **Schema-Valid Outputs**: 28
- **Success Rate**: **93.3%**
- **Average Latency**: 71.98s (p95)
- **Average Retries**: 0.97 per document
- **Average Cost**: $0.0007 per document
- **Total Evaluation Time**: 757.1s (~12.6 minutes)
...
| contract | 12 | 10 | 83.3% |
| chat_log | 9 | 9 | 100.0% |
| support_ticket | 9 | 9 | 100.0% |

## Failure Analysis

Distribution of failures across the pipeline:

- **wrong_type**: 56 occurrences
- **missing_field**: 79 occurrences
- **merge_conflict**: 9 occurrences
...
#### Document: `035a232e-4e44-445c-af34-7a661ff70293` (contract)
- **Retries Exhausted**: 3
- **Errors**:
  ...
  - [validation] wrong_type: Validation failed at path "clauses.1.clauseType": Invalid enum value. Expected 'termination' | 'confidentiality' | 'indemnification' | 'liability_limitation' | 'force_majeure' | 'dispute_resolution' | 'intellectual_property' | 'non_compete' | 'warranty' | 'amendment' | 'governing_law' | 'assignment' | 'other', received 'maintenance'
  - [validation] wrong_type: Validation failed at path "clauses.2.clauseType": Invalid enum value. Expected 'termination' | 'confidentiality' | 'indemnification' | 'liability_limitation' | 'force_majeure' | 'dispute_resolution' | 'intellectual_property' | 'non_compete' | 'warranty' | 'amendment' | 'governing_law' | 'assignment' | 'other', received 'insurance'
  - [validation] wrong_type: Validation failed at path "totalContractValue": Expected number, received null
  - [validation] wrong_type: Validation failed at path "parties.0.role": Invalid enum value. Expected 'buyer' | 'seller' | 'licensor' | 'licensee' | 'provider' | 'client' | 'other', received 'landlord'
  - [validation] wrong_type: Validation failed at path "parties.1.role": Invalid enum value. Expected 'buyer' | 'seller' | 'licensor' | 'licensee' | 'provider' | 'client' | 'other', received 'tenant'
  - [validation] wrong_type: Validation failed at path "clauses.1.clauseType": Invalid enum value. Expected 'termination' | 'confidentiality' | 'indemnification' | 'liability_limitation' | 'force_majeure' | 'dispute_resolution' | 'intellectual_property' | 'non_compete' | 'warranty' | 'amendment' | 'governing_law' | 'assignment' | 'other', received 'maintenance'
...
#### Document: `e8ddd791-eb3e-488d-a4a4-fb87ed1ac17a` (contract)
- **Retries Exhausted**: 3
- **Errors**:
  - [validation] wrong_type: Validation failed at path "expirationDate": Expected string, received null
  - [validation] wrong_type: Validation failed at path "paymentTerms.0.amount": Expected number, received null
  - [validation] wrong_type: Validation failed at path "paymentTerms.0.currency": Expected string, received null
  - [validation] wrong_type: Validation failed at path "paymentTerms.0.frequency": Expected 'one_time' | 'monthly' | 'quarterly' | 'annually' | 'upon_delivery' | 'milestone' | 'other', received null
  - [validation] wrong_type: Validation failed at path "paymentTerms.0.dueDate": Expected string, received null
  - [validation] wrong_type: Validation failed at path "paymentTerms.0.latePenaltyPercent": Expected number, received null
  - [validation] wrong_type: Validation failed at path "expirationDate": Expected string, received null
  - [validation] wrong_type: Validation failed at path "paymentTerms.0.amount": Expected number, received null
  - [validation] wrong_type: Validation failed at path "paymentTerms.0.currency": Expected string, received null
  ...
  - [validation] wrong_type: Validation failed at path "paymentTerms.0.dueDate": Expected string, received null
  - [validation] wrong_type: Validation failed at path "paymentTerms.0.latePenaltyPercent": Expected number, received null
  - [validation] missing_field: Validation failed at path "paymentTerms.0.amount": Required
  - [validation] missing_field: Validation failed at path "paymentTerms.0.amount": Required

### ✅ Sample Successful Extraction

**Document**: `ebfa8a75-2dfb-4cd7-a614-d41ee81a8953` (contract)
**Latency**: 7837ms | **Retries**: 0 | **Tokens**: 3722
